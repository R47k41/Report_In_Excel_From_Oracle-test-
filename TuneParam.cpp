...
NS_Tune::TImpDocsTune::TImpDocsTune(ptree& node, const string& main_path, const NS_Const::JsonParams& src_tag,
	...
	//инициализация параметров страницы и шаблонных значений:
	src_data(node, main_path, src_tag), 
	pattern(node, pattern_tag, ptrn_name_tag, ptrn_flds_tag, ptrn_blck_tag)
{
	//инициализация параметров полей для считывания из excel-файла
	...
	const NS_Const::JsonParams& ptrn_flds_tag, const NS_Const::JsonParams& ptrn_blck_tag) noexcept(true)
{
	...
	try
	{
		//инициализируем json-файл для считывания:
		ptree js;
		read_json(json_file, js);
		//если данных нет
		if (js.empty()) throw TLog("Пустые данные в файле: " + json_file, "TImpDocsTune::InitByJsonFile");
		//инициализация параметров страниц
		src_data = TShareData(js, main_path, src_tag);
		//инициализация параметров шаблонов:	
		pattern = TImpPatternTune(js, pattern_tag, ptrn_name_tag, ptrn_flds_tag, ptrn_blck_tag);
		//инициализация параметров ячеек строки страницы:
	...
	{
		TLog("Не обработанная ошибка при считывании json-файла: " + json_file, "TImpDocsTune::InitByJsonFile").toErrBuff();
	}
}
...
void NS_Tune::TImpDocsTune::show(std::ostream& stream) const noexcept(true)
{
	using std::endl;
	if (!stream) return;
	stream << "Параметры файлов для загрузки: ";
	if (src_data.isEmpty())
		stream << "не указаны!";
	else
		src_data.show(stream << '\n');
	stream << "Данные о параметрах считывания: ";
	if (params.empty())
